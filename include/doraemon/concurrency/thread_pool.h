...
#ifndef INCLUDE_DORAEMON_CONCURRENCY_THREAD_POOL_H_
#define INCLUDE_DORAEMON_CONCURRENCY_THREAD_POOL_H_
    ...
    explicit SimpleThreadPool(size_t count)
    {
        ...
        for (unsigned int i = 0; i < count; ++i) {
            std::shared_ptr<SingleThreadTaskExecutorService> e
              = std::make_shared<SingleThreadTaskExecutorService>();
            e->start();
...
#endif  // INCLUDE_DORAEMON_CONCURRENCY_THREAD_POOL_H_
